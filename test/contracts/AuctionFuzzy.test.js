...
contract("SuperAuction", accounts => {
  ...
  async function safeJoinAuction(account, flowRate) {
    if (await isPlayer(account)) {
  ...
  async function getAllUsers() {
    ...
    const allUsers = await Promise.all(
      accounts.map(async account => {
        ...
        console.log(`${userNames[account]} position: ${position}`);
      ...
      for (var i = 0; i < 25; i++) {
        ...
        async function doThings() {
          if (await isPlayer(user)) {
            ...
            await safeJoinAuction(user, Number(winnerFlowRate) + 10);
          }
        }
        // move time
